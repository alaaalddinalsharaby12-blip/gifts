...
<div class="container mx-auto py-10 px-4 max-w-6xl">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-2xl font-black text-gray-900">السمات (Attributes)</h2>
            <p class="text-gray-400 text-sm font-bold mt-1">إجمالي السمات: {{ $attributes->count() }}</p>
        </div>
        <a href="{{ route('admin.attributes.create') }}" class="btn-primary px-6 py-3 rounded-2xl font-black text-sm flex items-center gap-2 shadow-lg shadow-pink-200/50">
            ...
            <table class="min-w-full divide-y divide-gray-100">
                <thead>
                    <tr class="bg-gray-50/80">
                        ...
                        <th class="px-6 py-4 text-center text-xs font-black text-gray-400 uppercase tracking-wider">النوع</th>
                        <th class="px-6 py-4 text-right text-xs font-black text-gray-400 uppercase tracking-wider">القسم</th>
                        <th class="px-6 py-4 text-right text-xs font-black text-gray-400 uppercase tracking-wider">الخيارات</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="table-row">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-black text-gray-400">{{ $attr->id }}</td>
                        
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-black text-gray-900">{{ $attr->name }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            ...
                            @endphp
                            <span class="badge-type {{ $attr->type }} inline-flex items-center justify-center px-3 py-1 rounded-lg text-xs font-black">
                            ...
                            @if($attr->options && $attr->options->count() > 0)
                                <div class="flex flex-wrap gap-1.5 max-w-[400px]">
                                    @foreach($attr->options as $opt)
                                        <span class="option-badge inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold">
                                            @if($attr->type == 'color')
