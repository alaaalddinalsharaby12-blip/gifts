...
<div class="container mx-auto py-10 px-4 max-w-lg">
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h2 class="text-2xl font-black text-gray-900">إضافة قسم</h2>
            <p class="text-gray-400 text-sm font-bold">تنظيم المنتجات في تصنيفات</p>
        </div>
    </div>
    ...
    <div class="admin-card p-6 md:p-8">
        <form id="categoryForm" action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="مثال: صناديق الخطوبة"
            ...
            <div class="space-y-2">
                ...
                <textarea name="contents" rows="4" 
                        ...
                        placeholder="ورد طبيعي، شوكولاتة فاخرة، بالونات...">{{ old('contents') }}</textarea>
            </div>

            <!-- رفع الصورة -->
            <div class="space-y-2">
                <label class="block text-sm font-bold text-gray-700">صورة الغلاف <span class="text-red-500">*</span></label>
                <label id="imageUploadBox" class="upload-box flex items-center justify-center w-full py-4 rounded-xl">
                    <div class="flex items-center gap-2 text-gray-400 font-bold text-sm">
                        ...
                        <span id="imageFileName">اختيار صورة</span>
                    </div>
                    <input type="file" name="image" id="image" accept="image/*" class="hidden">
                </label>
                <p id="imageError" class="text-red-500 text-xs font-bold hidden text-center"></p>
            </div>

            <!-- رفع الفيديو -->
            <div class="space-y-2">
                <label class="block text-sm font-bold text-gray-700">فيديو القسم <span class="text-gray-400 text-xs font-normal">(يشتغل عند التمرير)</span></label>
                ...
                <label id="videoUploadBox" class="upload-box flex items-center justify-center w-full py-4 rounded-xl">
                    <div class="flex items-center gap-2 text-gray-400 font-bold text-sm">
                        ...
                        <span id="videoFileName">اختيار فيديو قصير</span>
                    </div>
...
document.getElementById('categoryForm').addEventListener('submit', function(e) {
    ...
    if (name.value.trim().length < 3) {
        document.getElementById('nameError').textContent = "الاسم قصير جداً (3 أحرف على الأقل)";
        document.getElementById('nameError').classList.remove('hidden');
        valid = false;
    }

    if (image.files.length === 0) {
        document.getElementById('imageError').textContent = "يجب اختيار صورة للقسم";
